...
public class Automobile implements Serializable{
	...
	public String getOptionSetName(String opset_name) throws AutoException {
		return this.findOpset(opset_name).getName();
	}
	...
	public ArrayList<String> getAllOpsetName() {
		ArrayList<String> opset_name = new ArrayList<String>();
		ArrayList<OptionSet> opset = this.getAllOpset();
		for(int i=0; i<opset.size(); i++ ) {
			opset_name.add(opset.get(i).getName());
		}
		return opset_name;
	}
	
	public ArrayList<String> getAllOptionName(String opset_name) {
		ArrayList<String> option_name = new ArrayList<String>();
		try {
			OptionSet opset = this.findOpset(opset_name);
			ArrayList<Option> option = opset.getAllOpt();
			for(int i=0; i<option.size(); i++) {
				option_name.add(option.get(i).getOptName());
			}
	...
	public ArrayList<String> getAllOptionPrice(String opset_name) {
		ArrayList<String> option_name = new ArrayList<String>();
		try {
			OptionSet opset = this.findOpset(opset_name);
			ArrayList<Option> option = opset.getAllOpt();
			for(int i=0; i<option.size(); i++) {
				option_name.add("" + option.get(i).getOptPrice());
			}
		} catch (AutoException e) {
